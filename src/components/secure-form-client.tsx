...
import { Lock, Shield, Clock, CheckCircle2, Upload, X } from "lucide-react";
...
export function SecureFormClient({
  ...
  // Client-side account number match check
      ...
      <main className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex items-center justify-center px-4 py-12">
        <div className="max-w-sm w-full text-center">
          ...
          <p className="text-slate-500 leading-relaxed mb-8">
            Your information has been encrypted and delivered to {agentName}. You can close this page.
          </p>
      ...
      <div className="max-w-sm mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-bold text-slate-900">
            {agentName}
            {agencyName && <span className="font-normal text-slate-500"> · {agencyName}</span>}
          </h1>
          {destinationLabel && (
            <p className="text-xs text-slate-400 mt-1.5">
              This submission will be delivered to: <span className="text-slate-600 font-medium">{destinationLabel}</span>
            </p>
          )}
        </div>
        ...
        <div className="flex items-center justify-center gap-5 mb-8">
          {[
            ...
            { icon: Clock, label: `Expires ${new Date(expiresAt).toLocaleDateString()}` },
          ].map(({ icon: Icon, label }) => (
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
          <p className="text-sm text-slate-500 mb-5 leading-relaxed">
            Enter your information below. Nothing is read aloud — this goes directly and privately to {agentName}.
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-1">
              <label className="flex items-start gap-3 cursor-pointer p-4 bg-slate-50 rounded-xl border border-slate-100">
                ...
                <span className="text-sm text-slate-600 leading-relaxed">
                  I consent to share this information with {agentName}
                  {agencyName ? ` (${agencyName})` : ""} for the purpose of completing my application. I understand it will be encrypted, retained for a limited period, and deleted afterward.
                </span>
              </label>
              {fieldErrors.consent && (
            ...
            <p className="text-xs text-slate-400 text-center leading-relaxed">
              This link cannot be reused after submission. Your information goes directly to {agentName} and is not shared with third parties.
            </p>
          </form>
        </div>
      </div>
